...
class MixtralAttention(nnx.Module):
    def __init__(self, layer_idx : int, config: MixtralConfig, dtype: jnp.dtype, rngs: nnx.Rngs):
        super().__init__()
        self.config = config
        self.dtype = dtype
...
class MixtralDecoderLayer(nnx.Module):
    def __init__(self, config: MixtralConfig, rngs : nnx.Rngs, layer_idx: int, dtype=jnp.float32):
        ...
        self.attn = MixtralAttention(config = config, dtype=dtype, rngs = rngs, layer_idx = layer_idx)
...
class FlaxMixtralForCausalLM(nnx.Module):
    ...
    def generate(
        ...
        # Key change: Pre-allocate a fixed-size array for all generated tokens
        # We know the maximum size will be initial tokens + max_new_tokens
        all_token_ids = jnp.zeros((batch_size, max_length), dtype=input_ids.dtype)
        # Copy the input tokens to our pre-allocated array
        all_token_ids = all_token_ids.at[:, :seq_length].set(input_ids)
        
        # Process the real prompt to fill in the cache for actual tokens
        outputs = self(
            input_ids=input_ids,
            attention_mask=attention_mask,
            past_key_values=past_key_values,
